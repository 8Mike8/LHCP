...
function LHCP_HandleTextEmote(event,arg1,arg2)
...
function LHCP_PlayIncomingText(event, arg1,arg2)
	if not LHCP_isEnabled then
		return nil;
	end
...
function LHCP_OnEvent(self, event, ...)
	if not LHCP_phrases then
		LHCP_Load();
	end
	
	LHCP_HandleEvent(self, event ,...);
end

function LHCP_HandleEvent(self, event, ...)
	--if LHCP_events then
	...
	if (event == "CHAT_MSG_SAY") then
		LHCP_PlayIncomingText(event, arg1,arg2);
	elseif (event == "CHAT_MSG_TEXT_EMOTE") then
		LHCP_HandleTextEmote(event,arg1,arg2);
